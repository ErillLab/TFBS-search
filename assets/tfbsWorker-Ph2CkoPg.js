...
`+(e.stack||``)})}}i(),self.onmessage=async e=>{console.log(`[WORKER] Recived:`,e.data);let{type:r,payload:i}=e.data;if(r===`init`&&(e.data.supportsSAB||(n=null)),r===`init-cancel-buffer`){n=new Uint8Array(e.data.buffer);return}if(r===`validate-genome`){let{filename:e,content:n}=i;try{t.FS.writeFile(`/tmp/${e}`,n),await t.runPythonAsync(`
...
Motif.load_motif("/tmp/motif_from_text.txt")
            `),postMessage({type:`motif-text-valid`,payload:{ok:!0,path:`/tmp/motif_from_text.txt`}})}catch(e){postMessage({type:`motif-text-valid`,payload:{ok:!1,error:e.toString()}})}}if(r===`run`){let{code:e,files:r}=i;n===null?await t.runPythonAsync(`
...
set_cancel_flag(False)
            `):(Atomics.store(n,0,0),t.globals.set(`_cancel_view`,n),await t.runPythonAsync(`
from tfbs.cancel_flag import set_cancel_view
set_cancel_view(_cancel_view)
from tfbs.cancel_flag import set_cancel_flag
set_cancel_flag(False)
            `));try{if(r)for(let e of r)t.FS.writeFile(e.path,e.content);let n=await t.runPythonAsync(e);postMessage({type:`result`,result:n})}catch(e){e.message?.includes(`PipelineCancelledError`)?postMessage({type:`cancelled`}):postMessage({type:`error`,error:e.toString()})}}if(r===`cancel`){n?n[0]=1:t.runPython(`
from tfbs.cancel_flag import set_cancel_flag
